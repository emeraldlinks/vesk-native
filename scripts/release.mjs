...
const scaffold = join(PACKAGES_DIR, 'create-native', 'src', 'index.ts');
if (existsSync(scaffold)) {
  ...
  if (stray.length > 0) {
    console.error('\x1b[31mrelease: create-vesk-native references an unpatched vesk version:\x1b[0m');
    for (const s of stray) console.error(`  ${s}`);
    process.exit(1);
  }
  writeFileSync(scaffold, patched);
  console.log('  \x1b[32m✓\x1b[0m create-vesk-native scaffold template versions');
}
