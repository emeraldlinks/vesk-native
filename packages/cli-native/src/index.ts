...
export { addLibrary, removeLibrary, updateLibraries, verifyApp, verifyBundle, installAllLibraries } from '@cli-native/commands';
...
async function main(): Promise<void> {
  ...
  switch (cmd) {
    ...
    case 'dev': {
      ...
      const mode = desktop ? 'desktop' : web ? 'web' : 'device';
      await devApp(cwd, Number.isFinite(port) ? port : 5173, mode);
      break;
    }
    default:
      usage();
